...
\begin{itemize}
\item The Random dataset consists of $1024$ input-output pairs, both consisting of $1024$ numbers drawn from a normal distribution with mean $0$ and variance $1$. We minimize the L2 loss. This tests the capacity of the network directly.
\item CIFAR-100 consists of $50000$ training image-label pairs, and $10000$ pairs for validation. Each pair has $3072$ inputs and $100$ one-hot outputs. We minimize the softmax loss (cross-entropy loss on a softmax activation), and also apply L1 regularization to the output with a multiplier of $10^-4$ for some reason. We do not apply any image-specific operations to inputs, and simply treat those as vectors of numbers, which tests capacity in isolation from generalization.
\end{itemize}

Each LDL cycles through dimensions and multiplies by a matrix each time, until the dimensions are back in the original order. Non-linearities in between those matrix multiplications significantly hurt performance, and should only be placed between whole layers. TODO: Show the result.

Internally, to get around severe hardware limitations, we have implemented this on top of TensorFlowJS. The creators of its WebGL backend decided to only support rank $6$ tensors; as such, we cannot test $n=2$, but only $n=16$ and above.

We do not share weight matrices across non-mixed dimension, because otherwise, there would be very few parameters.

    TODO: Have a table comparing runs (on the "Random" dataset) of linearithmic vs quadratic (mean and stddev of loss). (Sure hope that this trend of quadratic-is-worse continues into other experiments.) ("We intentionally tried to not over-think synthetic tests, so that it is more likely that they say something meaningful rather than a cherry-picked coincidence. This reduces the amount of computation that we need to perform.")

    TODO: Present CIFAR-100-based comparisons, once we have them?

    TODO: Compare with GAN-gating. If there is no difference, say so (ugh).

    TODO: Compare with RNN-based meta-learning.

    TODO: Run GAN-gated RNNs with no inputs. Hopefully, it won't be completely indistinguishable from random noise. "The main objective of this section is to justify the hypothesis that we can extract interesting representations from no data, only self-interaction. This regime separates approaches which take diversity from data (which is the bulk of modern machine learning) and approaches which make their own diversity."
